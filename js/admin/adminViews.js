  ...
  class AdminViews {
    ...
    render() {
      ...
      try {
        ...
        container.innerHTML = `
          <div class="admin-portal-wrapper" style="display: flex; flex-direction: column; height: 100%; min-height: 82vh; background: var(--bg-main, #f8fafc); color: var(--text-primary, #0f172a);">
            
            <!-- Admin Portal Top Header -->
            <div class="admin-top-header" style="display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.5rem; background: var(--bg-card, #ffffff); border-bottom: 1px solid var(--border-color, #e2e8f0); flex-wrap: wrap; gap: 0.75rem;">
              <div style="display: flex; align-items: center; gap: 0.75rem;">
                ...
                <div>
                  ...
                  <div style="font-size: 0.76rem; color: var(--text-muted, #64748b);">Samagra Shiksha ICT School Hi-Tech Labs & Smart Classrooms Monitoring System</div>
                </div>
              </div>

              <div style="display: flex; align-items: center; gap: 0.5rem;">
                <button type="button" onclick="window.adminViews.closeAdminPortal()" class="btn btn-sm btn-outline" style="font-weight: 700; border-radius: 8px; padding: 0.4rem 0.85rem; cursor: pointer;">
            ...
            <div class="admin-tabs-bar" style="display: flex; align-items: center; gap: 0.4rem; padding: 0.5rem 1.5rem; background: var(--bg-card, #ffffff); border-bottom: 1px solid var(--border-color, #e2e8f0); overflow-x: auto;">
    ...
    renderOverviewTab(calls, total, completed, inProgress, notStarted, criticalAging, totalDefects) {
      ...
      Object.keys(districtMap).forEach(d => {
        ...
        districtCardsHtml += `
          <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 12px; padding: 1rem; display: flex; flex-direction: column; gap: 0.5rem;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
              <div>
                <div style="font-weight: 800; font-size: 0.92rem; color: var(--text-primary, #0f172a);">${d}</div>
                <div style="font-size: 0.72rem; color: var(--text-muted, #64748b); display: flex; align-items: center; gap: 0.3rem;">
                  <i class="fas fa-user-check" style="color: #2563eb;"></i> Lead: <strong>${eng ? eng.name : 'Mohamed Shameer'}</strong>
                </div>
              </div>
            ...
            <div style="width: 100%; height: 6px; background: var(--bg-main, #f1f5f9); border-radius: 99px; overflow: hidden; margin: 0.2rem 0;">
              <div style="width: ${pct}%; height: 100%; background: linear-gradient(90deg, #3b82f6, #10b981); border-radius: 99px;"></div>
            </div>

            <div style="display: flex; justify-content: space-between; font-size: 0.73rem; color: var(--text-secondary, #475569);">
      ...
      return `
        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1rem;">
            <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.1rem;">
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b); font-weight: 700; text-transform: uppercase;">Total Registered Calls</div>
              <div style="font-size: 1.75rem; font-weight: 900; color: var(--text-primary, #0f172a); margin-top: 0.25rem;">${total}</div>
              <div style="font-size: 0.72rem; color: var(--text-secondary, #475569); margin-top: 0.2rem;">All Tamil Nadu Districts</div>
            </div>

            <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.1rem;">
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b); font-weight: 700; text-transform: uppercase;">Completed Tickets</div>
              <div style="font-size: 1.75rem; font-weight: 900; color: #10b981; margin-top: 0.25rem;">${completed}</div>
              <div style="font-size: 0.72rem; color: #10b981; margin-top: 0.2rem;">${total > 0 ? Math.round((completed/total)*100) : 0}% Statewide Resolution</div>
            </div>

            <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.1rem;">
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b); font-weight: 700; text-transform: uppercase;">In Progress</div>
              <div style="font-size: 1.75rem; font-weight: 900; color: #3b82f6; margin-top: 0.25rem;">${inProgress}</div>
              <div style="font-size: 0.72rem; color: var(--text-secondary, #475569); margin-top: 0.2rem;">Under Engineer Inspection</div>
            </div>

            <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.1rem;">
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b); font-weight: 700; text-transform: uppercase;">Not Started</div>
              <div style="font-size: 1.75rem; font-weight: 900; color: #f59e0b; margin-top: 0.25rem;">${notStarted}</div>
              <div style="font-size: 0.72rem; color: var(--text-secondary, #475569); margin-top: 0.2rem;">Awaiting Field Visit</div>
            </div>

            <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.1rem;">
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b); font-weight: 700; text-transform: uppercase;">Critical Aging (>100 Days)</div>
              <div style="font-size: 1.75rem; font-weight: 900; color: #ef4444; margin-top: 0.25rem;">${criticalAging}</div>
              <div style="font-size: 0.72rem; color: #ef4444; margin-top: 0.2rem;">Immediate Escalation Required</div>
            </div>

            <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.1rem;">
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b); font-weight: 700; text-transform: uppercase;">Hardware Defect Logs</div>
              <div style="font-size: 1.75rem; font-weight: 900; color: #8b5cf6; margin-top: 0.25rem;">${totalDefects}</div>
              <div style="font-size: 0.72rem; color: var(--text-secondary, #475569); margin-top: 0.2rem;">Pending Hardware Requisitions</div>
            </div>
          </div>

          <!-- District Distribution Grid -->
          <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.25rem;">
            <div style="font-size: 0.95rem; font-weight: 800; color: var(--text-primary, #0f172a); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
              <i class="fas fa-map-location-dot" style="color: #2563eb;"></i> District Operations Breakdown & Engineer Assignment
            </div>
    ...
    renderEngineersTab() {
      ...
      engineers.forEach(eng => {
        ...
        engRows += `
          <tr style="border-bottom: 1px solid var(--border-color, #e2e8f0);">
            <td style="padding: 0.85rem 1rem; font-weight: 800; font-size: 0.82rem; color: var(--text-primary, #0f172a);">
              #${eng.empId}
            </td>
            <td style="padding: 0.85rem 1rem;">
              <div style="font-weight: 700; font-size: 0.85rem; color: var(--text-primary, #0f172a);">${eng.name}</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">${eng.email} • 📞 ${eng.phone}</div>
            </td>
            <td style="padding: 0.85rem 1rem; font-size: 0.8rem; font-weight: 600; color: var(--text-secondary, #475569);">
              📍 ${eng.baseLocation}
            </td>
            <td style="padding: 0.85rem 1rem;">
            ...
            <td style="padding: 0.85rem 1rem; text-align: right;">
              <div style="display: flex; align-items: center; justify-content: flex-end; gap: 0.4rem;">
                <button type="button" onclick="window.adminViews.openAssignDistrictsModal('${eng.empId}')" class="btn btn-xs btn-outline" style="font-size: 0.7rem; font-weight: 700; padding: 0.25rem 0.55rem; cursor: pointer;" title="Assign/Change Districts">
                  <i class="fas fa-map-pin"></i> Assign Districts
                </button>
                <button type="button" onclick="window.adminViews.openResetPinModal('${eng.empId}')" class="btn btn-xs btn-outline" style="font-size: 0.7rem; font-weight: 700; color: #ef4444; border-color: rgba(239,68,68,0.3); padding: 0.25rem 0.55rem; cursor: pointer;" title="Reset Login PIN">
      ...
      return `
        <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; overflow: hidden;">
          <div style="padding: 1.25rem; border-bottom: 1px solid var(--border-color, #e2e8f0); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem;">
            <div>
              <div style="font-size: 1rem; font-weight: 800; color: var(--text-primary, #0f172a);">Field Engineers & District Dispatch Roster</div>
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b);">Manage field engineers handling 1, 2, or 3 districts and reset credentials</div>
            </div>
          </div>
          ...
          <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
              <thead>
                <tr style="background: var(--bg-main, #f8fafc); border-bottom: 1px solid var(--border-color, #e2e8f0); font-size: 0.74rem; font-weight: 800; text-transform: uppercase; color: var(--text-muted, #64748b);">
    ...
    renderSchoolsTab() {
      const store = window.adminStore || {};
      const schools = (store.state && Array.isArray(store.state.schoolsMaster)) ? store.state.schoolsMaster : [];

      let rows = '';
      schools.forEach(s => {
        const catBadge = s.category === 'PUPS' ? 'background: rgba(245,158,11,0.1); color: #d97706;' : 'background: rgba(37,99,235,0.1); color: #2563eb;';
        const labBadge = s.labCount > 1 ? `background: rgba(139,92,246,0.1); color: #8b5cf6; font-weight: 800;` : `background: var(--bg-main, #f1f5f9); color: var(--text-secondary, #475569);`;

        rows += `
          <tr style="border-bottom: 1px solid var(--border-color, #e2e8f0);">
            <td style="padding: 0.85rem 1rem; font-family: monospace; font-size: 0.8rem; font-weight: 800; color: #2563eb;">
              ${s.udise}
            </td>
            <td style="padding: 0.85rem 1rem;">
              <div style="font-weight: 700; font-size: 0.85rem; color: var(--text-primary, #0f172a);">${s.schoolName}</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">${s.district} • Block: ${s.block}</div>
            </td>
            <td style="padding: 0.85rem 1rem;">
            ...
            <td style="padding: 0.85rem 1rem;">
              <div style="font-weight: 700; font-size: 0.82rem; color: var(--text-primary, #0f172a);">${s.aiName}</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">AI (Administrator-cum-Instructor)</div>
            ...
            <td style="padding: 0.85rem 1rem; text-align: right;">
              <a href="tel:${s.aiPhone}" class="btn btn-xs btn-outline" style="font-size: 0.7rem; font-weight: 700; padding: 0.25rem 0.55rem; text-decoration: none; display: inline-flex; align-items: center; gap: 0.3rem; cursor: pointer;">
      ...
      return `
        <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; overflow: hidden;">
          <div style="padding: 1.25rem; border-bottom: 1px solid var(--border-color, #e2e8f0); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem;">
            <div>
              <div style="font-size: 1rem; font-weight: 800; color: var(--text-primary, #0f172a);">Tamil Nadu School Directory & AI Contacts</div>
              <div style="font-size: 0.75rem; color: var(--text-muted, #64748b);">UDISE Codes, School Categories (PUMS, PUPS, GHSS, HSS), Lab Counts & Administrator-cum-Instructor Contacts</div>
            </div>
          </div>
    ...
    renderAssetsTab() {
      const store = window.adminStore || {};
      const defects = (store.state && Array.isArray(store.state.defectLogs)) ? store.state.defectLogs : [];

      let defectRows = '';
      defects.forEach(d => {
        defectRows += `
          <tr style="border-bottom: 1px solid var(--border-color, #e2e8f0);">
            <td style="padding: 0.85rem 1rem; font-size: 0.78rem; font-weight: 700; color: var(--text-muted, #64748b);">${d.reportedDate}</td>
            <td style="padding: 0.85rem 1rem;">
              <div style="font-weight: 700; font-size: 0.85rem; color: var(--text-primary, #0f172a);">${d.schoolName}</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">${d.district} • UDISE: ${d.udise} • <strong>${d.labNo}</strong></div>
            </td>
            <td style="padding: 0.85rem 1rem; font-weight: 800; font-size: 0.82rem; color: var(--text-primary, #0f172a);">
              ${d.qty}x ${d.itemName}
            </td>
            ...
            <td style="padding: 0.85rem 1rem; font-size: 0.75rem; color: var(--text-secondary, #475569); max-width: 250px;">
      ...
      return `
        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
          ...
          <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; overflow: hidden;">
            <div style="padding: 1.25rem; border-bottom: 1px solid var(--border-color, #e2e8f0); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem;">
              <div>
                <div style="font-size: 1rem; font-weight: 800; color: var(--text-primary, #0f172a);">Live Hardware Defect & Replacement Log</div>
                <div style="font-size: 0.75rem; color: var(--text-muted, #64748b);">Chain-reaction defect records reported by field engineers during school inspections</div>
              </div>
            </div>
      ...
      return `
        <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.5rem; display: flex; flex-direction: column; gap: 1.25rem;">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
            <div>
              <div style="font-size: 1.05rem; font-weight: 800; color: var(--text-primary, #0f172a);">Calendar Historical Reports & Executive Export</div>
              <div style="font-size: 0.76rem; color: var(--text-muted, #64748b);">Query historical school calls, engineer performance, and hardware defects across any custom date range</div>
            </div>
            <button type="button" onclick="if(window.exporter && window.exporter.exportToExcel) window.exporter.exportToExcel();" class="btn btn-sm btn-primary" style="background: linear-gradient(135deg, #107c41, #0b5e31); font-weight: 700; border-radius: 8px; cursor: pointer;">
              <i class="fas fa-file-excel"></i> Export Master Excel (.xlsx)
            </button>
          </div>

          <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap; padding: 1rem; background: var(--bg-main, #f8fafc); border-radius: 10px;">
            <div style="display: flex; flex-direction: column; gap: 0.3rem;">
              <label style="font-size: 0.72rem; font-weight: 800; color: var(--text-muted, #64748b); text-transform: uppercase;">From Date</label>
              <input type="date" id="adminReportStartDate" class="form-input" style="padding: 0.4rem 0.6rem; font-size: 0.8rem; border-radius: 6px;">
            </div>
            <div style="display: flex; flex-direction: column; gap: 0.3rem;">
              <label style="font-size: 0.72rem; font-weight: 800; color: var(--text-muted, #64748b); text-transform: uppercase;">To Date</label>
            ...
            <div style="display: flex; align-items: flex-end; padding-top: 1.2rem;">
              <button type="button" onclick="alert('Filtered ' + (window.appStore ? window.appStore.calls.length : 0) + ' historical calls for selected period.');" class="btn btn-sm btn-primary" style="padding: 0.45rem 0.9rem; font-weight: 700; border-radius: 6px; cursor: pointer;">
                <i class="fas fa-filter"></i> Apply Date Filter
              </button>
            </div>
          </div>

          <div style="padding: 1.5rem; text-align: center; border: 1.5px dashed var(--border-color, #e2e8f0); border-radius: 12px; color: var(--text-secondary, #475569); font-size: 0.85rem;">
            <i class="fas fa-database" style="font-size: 2rem; color: #2563eb; display: block; margin-bottom: 0.5rem;"></i>
    ...
    renderSearchTab(calls) {
      return `
        <div style="background: var(--bg-card, #ffffff); border: 1px solid var(--border-color, #e2e8f0); border-radius: 14px; padding: 1.5rem; display: flex; flex-direction: column; gap: 1.25rem;">
          <div>
            <div style="font-size: 1.05rem; font-weight: 800; color: var(--text-primary, #0f172a);">Universal Search Engine</div>
            <div style="font-size: 0.76rem; color: var(--text-muted, #64748b);">Instant search by Field Engineer Name, Employee ID, District, School UDISE, Category, or Equipment Type</div>
          </div>

          <div style="position: relative;">
            <i class="fas fa-search" style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: var(--text-muted, #64748b);"></i>
            <input type="text" id="adminUniversalSearchInput" oninput="window.adminViews.handleUniversalSearch(this.value)" placeholder="Search any UDISE (e.g. 33190400501), Engineer (Shameer, 569), District (Nagapattinam), or School..." style="width: 100%; padding: 0.75rem 1rem 0.75rem 2.75rem; border-radius: 10px; border: 1px solid var(--border-color, #e2e8f0); background: var(--bg-main, #f8fafc); font-size: 0.88rem; color: var(--text-primary, #0f172a);">
          </div>

          <div id="adminSearchResults" style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 450px; overflow-y: auto;">
            <div style="color: var(--text-muted, #64748b); font-size: 0.8rem; text-align: center; padding: 2rem;">
    ...
    handleUniversalSearch(query) {
      ...
      const matchedSchools = (state.schoolsMaster || []).filter(s => 
        (s.schoolName && s.schoolName.toLowerCase().includes(q)) || (s.udise && s.udise.includes(q)) || (s.aiName && s.aiName.toLowerCase().includes(q)) || (s.aiPhone && s.aiPhone.includes(q))
      );
      ...
      if (matchedEngineers.length > 0) {
        html += '<div style="font-weight: 800; font-size: 0.78rem; color: #2563eb; text-transform: uppercase;">Field Engineers Matched:</div>';
        matchedEngineers.forEach(eng => {
          html += `
            <div style="padding: 0.75rem; background: var(--bg-main, #f8fafc); border: 1px solid var(--border-color, #e2e8f0); border-radius: 8px;">
              <div style="font-weight: 700; font-size: 0.85rem; color: var(--text-primary, #0f172a);">${eng.name} (Emp ID: ${eng.empId})</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">Assigned Districts: <strong>${(eng.assignedDistricts || []).join(', ')}</strong> • PIN: ${eng.pin}</div>
            </div>
          `;
        });
      }
      ...
      if (matchedSchools.length > 0) {
        ...
        matchedSchools.forEach(s => {
          html += `
            <div style="padding: 0.75rem; background: var(--bg-main, #f8fafc); border: 1px solid var(--border-color, #e2e8f0); border-radius: 8px;">
              <div style="font-weight: 700; font-size: 0.85rem; color: var(--text-primary, #0f172a);">${s.schoolName} (${s.category}) • UDISE: ${s.udise}</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">AI: <strong>${s.aiName}</strong> (📞 ${s.aiPhone}) • ${s.labCount} Hi-Tech Lab(s)</div>
            </div>
          `;
        });
      }
      ...
      if (matchedCalls.length > 0) {
        ...
        matchedCalls.slice(0, 10).forEach(c => {
          html += `
            <div style="padding: 0.75rem; background: var(--bg-main, #f8fafc); border: 1px solid var(--border-color, #e2e8f0); border-radius: 8px;">
              <div style="font-weight: 700; font-size: 0.85rem; color: var(--text-primary, #0f172a);">${c.schoolName || 'School'} [${c.status}]</div>
              <div style="font-size: 0.72rem; color: var(--text-muted, #64748b);">Issue: ${c.issue || 'N/A'} • District: ${c.district || 'NAGAPATTINAM'} • Age: ${c.ageDays || 0} days</div>
            </div>
          `;
        });
      }

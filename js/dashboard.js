...
class FieldCallDashboard {
  ...
  updateDashboard(calls, settings) {
    ...
    const completedCalls = calls.filter(c => norm(c.status) === 'completed').length;
    const inProgressCalls = calls.filter(c => norm(c.status) === 'in progress').length;
    ...
    calls.forEach(c => {
      const dist = parseNum(c.distanceKm);
      const costStored = parseNum(c.conveyanceCost);

      if (dist !== null && dist > 0) {
        totalDistance += dist;
        const cost = (costStored !== null && costStored >= 0) ? costStored : Math.round(dist * rate);
        totalConveyance += cost;
      } else if (costStored !== null && costStored > 0) {
        totalConveyance += costStored;
      }
    });
    ...
    this.setElementText('kpiConveyanceSubtext', `${totalDistance.toLocaleString('en-IN')} km total traveled`);
    ...
    if (tableBody) {
      ...
      blocks.forEach(b => {
        ...
        const bComp = bCalls.filter(c => norm(c.status) === 'completed').length;
